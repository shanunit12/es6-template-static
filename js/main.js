    ...
    render(){
        $.get(`${this.sBase}/items/${this.oItems[this.nCurrentItem].fname}`, (sMarkdown) => {
            $("article#current").append(`
                <div class="itemImage"><img alt="image" src="${this.getImageSrc(this.oItems[this.nCurrentItem].specialImage)}" /></div>
            `);
            $("article#current").append(`
        ...
        for(let n = 0; n < this.oItems.length; n++){
            if(n != this.nCurrentItem){
                ...
                <div class="item">
                    <a class="itemLink" href="#"><img alt="image" id="item${n}" src="${this.getImageSrc(this.oItems[n].specialImage)}" /></a>
